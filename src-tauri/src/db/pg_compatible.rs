...
fn full_table(table: &str, schema: Option<&str>) -> String {
    match schema {
...
fn full_table_quoted(table: &str, schema: Option<&str>) -> String {
...
impl DatabaseConnection for PgCompatibleConnection {
    ...
    async fn get_table_row_count(
        ...
        let full_table_ref = full_table(table, schema);
    ...
    async fn update_table_rows(
        ...
        let full_table_ref = full_table_quoted(table, schema);
        let set_clauses: Vec<String> = updates
            .iter()
            .map(|(col, val)| {
                format!(
                    "{} = {}",
                    escape_identifier(col, &DatabaseType::PostgreSQL),
