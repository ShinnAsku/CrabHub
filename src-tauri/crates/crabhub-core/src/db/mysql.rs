...
impl MySqlConnection {
    pub async fn new(config: &ConnectionConfig) -> Result<Self, DbError> {
        ...
        let port = config.port.unwrap_or(config.db_type.default_port());
        ...
        let connection_string = if config.ssl_enabled && config.db_type != DatabaseType::MySQL {
            connection_string.replace("ssl-mode=preferred", "ssl-mode=verify_identity")
        ...
        let options: sqlx::mysql::MySqlConnectOptions = connection_string.parse()
            .map_err(|_| DbError::ConfigError("Invalid MySQL connection options".into()))?;
        ...
        Ok(Self { pool, kind: config.db_type.clone(), connection_string, session_ids, script_sessions: config.db_type == DatabaseType::MySQL })
    }
}
...
impl MySqlConnection {
    ...
    pub(crate) async fn query_on(connection: &mut sqlx::MySqlConnection, sql: &str) -> Result<QueryResult, DbError> {
        let start = Instant::now();
        ...
        if result.is_empty() {
            use sqlx::Executor;
            let description = connection.describe(sql).await.map_err(DbError::from)?;
            return Ok(QueryResult {
                columns: description.columns().iter().enumerate().map(|(index, column)| ColumnInfo {
                    name: column.name().to_string(), data_type: column.type_info().name().to_string(),
                    nullable: description.nullable(index).unwrap_or(true), is_primary_key: false,
                    default_value: None, comment: None, character_maximum_length: None,
    ...
    pub(crate) fn decode_rows(result: &[sqlx::mysql::MySqlRow], elapsed: u64) -> QueryResult {
        let columns = result.first().map(build_columns_from_mysql_row).unwrap_or_default();
...
impl DatabaseConnection for MySqlConnection {
    fn supports_script_sessions(&self) -> bool { self.script_sessions }

    async fn execute_sql(&self, sql: &str) -> Result<ExecuteResult, DbError> {
        MySqlConnection::execute_sql(self, sql).await
    }

    async fn query_sql(&self, sql: &str) -> Result<QueryResult, DbError> {
        MySqlConnection::query_sql(self, sql).await
    }

    async fn open_session(&self) -> Result<Box<dyn super::session::DatabaseSession>, DbError> {
        let mut connection = self.pool.acquire().await.map_err(super::session::sqlx_error)?;
        connection.close_on_drop();
        let id: u64 = sqlx::query_scalar("SELECT CONNECTION_ID()").fetch_one(&mut *connection).await.map_err(super::session::sqlx_error)?;
        self.session_ids.lock().unwrap().retain(|tracked| *tracked != id);
        let cancel = super::session_cancel::SessionCancel::MySql { connection_string: self.connection_string.clone(), id };
        Ok(Box::new(super::session::SqlxSession::with_cancel(connection, cancel)))
    }
        ...
        let tables = rows
            ...
            .map(|row| {
                ...
                Ok(TableInfo {
                    ...
                    row_count: row.try_get::<Option<i64>, _>("table_rows")?.and_then(|count| u64::try_from(count).ok()),
                    comment: row
    ...
    async fn get_columns(
        ...
        let columns = rows
            .iter()
            .map(|row| -> Result<ColumnInfo, sqlx::Error> {
                let is_nullable: String = row.try_get("is_nullable")?;
                let column_key: String = row.try_get("column_key")?;
                let char_max_len: Option<i64> = row.try_get("char_max_length")?;
                let num_precision: Option<i64> = row.try_get("num_precision")?;
                let num_scale: Option<i64> = row.try_get("num_scale")?;
                Ok(ColumnInfo {
            ...
            .map_err(|error| DbError::QueryError(format!("MySQL column metadata: {error}")))?;
    ...
    async fn get_views(&self, schema: Option<&str>) -> Result<Vec<TableInfo>, DbError> {
        ...
        let sql = format!(
            "SELECT CAST(TABLE_NAME AS CHAR) as table_name, CAST(TABLE_SCHEMA AS CHAR) as table_schema, \
             CAST(NULL AS CHAR) as table_comment, 'VIEW' as table_type \
    ...
    async fn delete_table_rows(
        ...
        let sql = format!("DELETE FROM {} WHERE {}", full_table, where_sql);
        self.execute_sql(&sql).await
    }
}
