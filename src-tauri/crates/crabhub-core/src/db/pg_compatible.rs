...
use super::pg_utils;
use super::trait_def::{escape_identifier, json_value_to_sql, DatabaseConnection};
use super::types::{
...
impl PgCompatibleConnection {
    pub async fn new(config: &ConnectionConfig, dialect: DialectConfig) -> Result<Self, DbError> {
        ...
        let ssl_mode = if config.ssl_enabled { "verify-full" } else { "prefer" };
        let conn_str = format!(
        ...
        let options: sqlx::postgres::PgConnectOptions = conn_str.parse().map_err(|_| DbError::ConfigError("Invalid PostgreSQL-compatible connection options".into()))?;
...
impl DatabaseConnection for PgCompatibleConnection {
    ...
    async fn query_sql(&self, sql: &str) -> Result<QueryResult, DbError> {
        ...
        let (mut columns, rows) = pg_utils::parse_pg_rows(&result);
        if result.is_empty() {
            use sqlx::{Executor, TypeInfo};
            let description = self.pool.describe(sql).await.map_err(|error| DbError::QueryError(error.to_string()))?;
            columns = description.columns().iter().enumerate().map(|(index, column)| ColumnInfo {
                ...
                default_value: None, comment: None, character_maximum_length: None, numeric_precision: None, numeric_scale: None,
            }).collect();
        }
    ...
    async fn get_columns(
        ...
        let sql = "SELECT c.column_name, c.data_type, c.is_nullable, c.column_default, \
                   ...
                   CAST(c.numeric_precision AS BIGINT) AS numeric_precision, CAST(c.numeric_scale AS BIGINT) AS numeric_scale, \
    ...
    async fn get_views(&self, schema: Option<&str>) -> Result<Vec<TableInfo>, DbError> {
        ...
        let views_sql = format!("SELECT * FROM ({sql}) AS visible_views WHERE ($1::text IS NULL OR table_schema = $1) ORDER BY table_schema, table_name");
    ...
    async fn get_table_row_count(
        ...
        let count: i64 = sqlx::query_scalar(&sql).fetch_one(&self.pool).await.map_err(|error| DbError::QueryError(error.to_string()))?;
        u64::try_from(count).map_err(|_| DbError::QueryError("Invalid negative row count".into()))
    }
