...
pub(crate) fn returns_rows(statement: &str) -> Result<bool, DbError> {
    Ok(!matches!(parse_stmt(statement)?, MongoStmt::InsertOne { .. } | MongoStmt::DeleteMany { .. }))
}

#[cfg(test)]
mod routing_tests {
    #[test]
    fn mongo_editor_commands_use_mongo_syntax() {
        for sql in ["db.records.find({})", "db.records.aggregate([])", "db.records.countDocuments({})", "show collections"] {
            assert!(super::returns_rows(sql).unwrap(), "{sql}");
        }
        for sql in ["db.records.insertOne({\"value\":\"find\"})", "db.records.deleteMany({})"] {
            assert!(!super::returns_rows(sql).unwrap(), "{sql}");
        }
        assert!(super::returns_rows("db.records.find({invalid json})").is_err());
    }
}
