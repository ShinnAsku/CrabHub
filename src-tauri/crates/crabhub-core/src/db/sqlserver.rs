...
impl SqlServerConnection {
    pub async fn new(config: &ConnectionConfig) -> Result<Self, DbError> {
        ...
        tc.encryption(if config.ssl_enabled {
                    ...
                    for name in &col_names {
                        let val: serde_json::Value = row.try_get::<&str, _>(name.as_str()).ok().flatten()
                            ...
                            .or_else(|| row.try_get::<i64, _>(name.as_str()).ok().flatten()
                                .map(|v| serde_json::Value::Number(serde_json::Number::from(v))))
                            .or_else(|| row.try_get::<tiberius::numeric::Numeric, _>(name.as_str()).ok().flatten()
...
impl DatabaseConnection for SqlServerConnection {
    async fn execute_sql(&self, sql: &str) -> Result<ExecuteResult, DbError> {
        ...
        let result = client.execute(sql, &[]).await
            .map_err(|e| DbError::QueryError(format!("SQLServer: {}", e)))?;
        Ok(ExecuteResult { rows_affected: result.total(), execution_time_ms: start.elapsed().as_millis() as u64 })
    }
    ...
    async fn get_columns(&self, table: &str, schema: Option<&str>) -> Result<Vec<ColumnInfo>, DbError> {
        ...
        let rows = self.query_strings(&format!(
            "SELECT c.COLUMN_NAME, c.DATA_TYPE, c.IS_NULLABLE, \
             CAST(CASE WHEN EXISTS (SELECT 1 FROM INFORMATION_SCHEMA.TABLE_CONSTRAINTS tc \
             JOIN INFORMATION_SCHEMA.KEY_COLUMN_USAGE kc ON tc.CONSTRAINT_CATALOG=kc.CONSTRAINT_CATALOG \
             AND tc.CONSTRAINT_SCHEMA=kc.CONSTRAINT_SCHEMA AND tc.CONSTRAINT_NAME=kc.CONSTRAINT_NAME \
             WHERE tc.CONSTRAINT_TYPE='PRIMARY KEY' AND kc.TABLE_CATALOG=c.TABLE_CATALOG \
             AND kc.TABLE_SCHEMA=c.TABLE_SCHEMA AND kc.TABLE_NAME=c.TABLE_NAME AND kc.COLUMN_NAME=c.COLUMN_NAME) \
        ...
        Ok(rows.iter().map(|row| ColumnInfo {
            ...
            is_primary_key: row.get("IS_PRIMARY_KEY").and_then(|value| value.as_bool()).unwrap_or(false),
            default_value: None, comment: None,
            character_maximum_length: None, numeric_precision: None, numeric_scale: None,
        }).collect())
    }
