...
impl PostgresConnection {
    pub async fn new(config: &ConnectionConfig) -> Result<Self, DbError> {
        ...
        let options: sqlx::postgres::PgConnectOptions = connection_string.parse()
            .map_err(|_| DbError::ConfigError("Invalid PostgreSQL connection options".into()))?;
...
impl PostgresConnection {
    pub(crate) async fn query_on(connection: &mut sqlx::PgConnection, sql: &str) -> Result<QueryResult, DbError> {
        let start = Instant::now();
        let result = sqlx::query(sql).fetch_all(&mut *connection).await
            .map_err(|error| DbError::QueryError(error.to_string()))?;
        Ok(Self::decode_rows(&result, start.elapsed().as_millis() as u64))
    }

    pub(crate) fn decode_rows(result: &[sqlx::postgres::PgRow], elapsed: u64) -> QueryResult {
...
impl DatabaseConnection for PostgresConnection {
    fn supports_script_sessions(&self) -> bool { true }
    ...
    async fn open_session(&self) -> Result<Box<dyn super::session::DatabaseSession>, DbError> {
        ...
        let application_name = format!("crabhub-script-{}", uuid::Uuid::new_v4());
        let (pid, _): (i32, String) = sqlx::query_as("SELECT pg_backend_pid(), set_config('application_name', $1, false)")
            .bind(&application_name).fetch_one(&mut *connection).await.map_err(super::session::sqlx_error)?;
        let cancel = super::session_cancel::SessionCancel::PostgreSql { connection_string: self.connection_string.clone(), pid, application_name };
