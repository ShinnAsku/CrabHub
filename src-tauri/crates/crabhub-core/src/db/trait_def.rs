...
pub trait DatabaseConnection: Send + Sync {
    fn handles_query_pagination(&self) -> bool { false }

    async fn database_names(&self) -> Result<Option<Vec<String>>, DbError> { Ok(None) }

    async fn ping(&self) -> Result<(), DbError> {
        self.query_sql("SELECT 1").await.map(|_| ())
    }

    fn supports_script_sessions(&self) -> bool { false }

    async fn open_session(&self) -> Result<Box<dyn super::session::DatabaseSession>, DbError> {
        Err(DbError::QueryError("Isolated script sessions are not supported by this driver".into()))
    }
...
pub fn escape_identifier(ident: &str, db_type: &crate::db::types::DatabaseType) -> String {
    match db_type {
        ...
        | crate::db::types::DatabaseType::TiDB | crate::db::types::DatabaseType::OceanBase
        | crate::db::types::DatabaseType::TDSQL => {
