...
//! One physical client; pipelining does not provide server-side parallelism.
...
pub struct GaussAsyncConnection {
    client: tokio::sync::Mutex<GClient>,
...
impl GaussAsyncConnection {
    pub async fn new(config: &ConnectionConfig) -> Result<Self, DbError> {
        if config.ssl_enabled {
            return Err(DbError::ConfigError("TLS is not implemented by the current GaussDB adapter; refusing to downgrade to plaintext".into()));
        }
        ...
        if config.pool_options.is_some() {
            log::warn!(
                "[gauss_rs] pool_options provided for connection '{}' but the tokio-gaussdb driver \
                 ...
                 Actual execution capacity is one physical client.",
                config.name
            );
        }
        ...
        Ok(Self { client: tokio::sync::Mutex::new(client), task, cancel_token })
    }
}

impl Drop for GaussAsyncConnection {
...
impl DatabaseConnection for GaussAsyncConnection {
    async fn query_sql(&self, sql: &str) -> Result<QueryResult, DbError> {
        ...
        let messages = self.client.lock().await.simple_query(sql).await.map_err(|e| DbError::QueryError(e.to_string()))?;
    ...
    async fn execute_sql(&self, sql: &str) -> Result<ExecuteResult, DbError> {
        let start = std::time::Instant::now();
        let affected = self.client.lock().await.execute(sql, &[]).await.map_err(|e| DbError::QueryError(e.to_string()))?;
    ...
    async fn close(&self) { self.task.abort(); }

    async fn get_tables(&self) -> Result<Vec<TableInfo>, DbError> {
        self.query_sql("SELECT t.table_schema, t.table_name, t.table_type \
            ...
            ORDER BY t.table_schema, t.table_name").await.map(|r| {
    ...
    async fn get_schemas(&self) -> Result<Vec<String>, DbError> {
        let sql = "SELECT nspname FROM pg_catalog.pg_namespace WHERE nspname NOT LIKE 'pg_%' AND nspname != 'information_schema' AND has_schema_privilege(oid, 'USAGE') ORDER BY nspname";
    ...
    async fn get_views(&self, schema: Option<&str>) -> Result<Vec<TableInfo>, DbError> {
        let f = schema.map(|s| format!("AND v.table_schema='{}'", crate::db::trait_def::escape_sql_string(s))).unwrap_or_default();
        self.query_sql(&format!("SELECT v.table_schema, v.table_name FROM information_schema.views v \
            ...
            AND has_schema_privilege(n.oid, 'USAGE') AND has_table_privilege(c.oid, 'SELECT') \
            {} ORDER BY v.table_schema, v.table_name", f)).await.map(|r| {
            r.rows.iter().map(|row| TableInfo {
                name: row.get("table_name").and_then(|v| v.as_str().map(String::from)).unwrap_or_default(),
                schema: row.get("table_schema").and_then(|v| v.as_str().map(String::from)),
    ...
    async fn get_table_row_count(&self, table: &str, schema: Option<&str>) -> Result<u64, DbError> {
        ...
        let result = self.query_sql(&format!("SELECT COUNT(*) as cnt FROM \"{}\".\"{}\"", s_safe, t_safe)).await?;
        result.rows.first().and_then(|row| row.get("cnt"))
            .and_then(JsonValue::as_str).and_then(|value| value.parse::<u64>().ok())
            .ok_or_else(|| DbError::QueryError("GaussDB returned an invalid table row count".into()))
    }
        ...
        match msg {
            SimpleQueryMessage::Row(row) => {
                ...
                for (index, column) in row.columns().iter().enumerate() {
                    let value = row.try_get::<usize>(index)
                        .map_err(|error| DbError::QueryError(format!("GaussDB result decoding failed: {error}")))?
