...
impl AgentRuntime {
    ...
    pub fn check_safety(tool_name: &str, params: &serde_json::Value) -> SafetyAction {
        ...
        } else if matches!(tool_name, "execute_select" | "explain_query") {
            let sql = params["sql"].as_str().unwrap_or("");
            if crate::db::sql_kind::parse_one(sql)
                .is_ok_and(|statement| crate::db::sql_kind::is_read_only(&statement)) {
                SafetyAction::Allow
            } else {
                SafetyAction::Deny { reason: "Read-only tools require a verified single SELECT query".into() }
            }
        } else {
            SafetyAction::Allow
        }
                ...
                for tc in tool_calls {
                    ...
                    match safety {
                        ...
                        SafetyAction::Confirm { reason } => {
                            let _ = tx.send(AgentEvent::NeedsConfirmation {
                                sql: params["sql"].as_str().unwrap_or("").to_string(),
                                reason,
                            }).await;
                            return Ok(());
                        }
