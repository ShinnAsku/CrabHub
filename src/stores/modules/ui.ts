...
import {
  ...
  initLanguage as initI18nLanguage,
  type Language,
} from '@/lib/i18n'
...
export const useUIStore = create<UIState>(set => ({
  ...
  toggleTheme: () =>
    set(state => {
      ...
      try {
        localStorage.setItem('crabhub-theme', newTheme)
      } catch {}
  ...
  setTheme: theme =>
    set(() => {
      ...
      try {
        localStorage.setItem('crabhub-theme', theme)
      } catch {}
      return { theme }
    }),
  setLanguage: lang =>
  ...
  toggleSnippetPanel: () => set(state => ({ snippetPanelOpen: !state.snippetPanelOpen })),
