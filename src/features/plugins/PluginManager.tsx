...
function PluginCard({
  ...
  return (
    <div
      className={`group relative flex h-full flex-col rounded-lg bg-card overflow-hidden transition-all duration-200 ease-out hover:-translate-y-px
    ...
    >
      {showBand && (
        <div className={`flex h-12 shrink-0 items-center justify-center ${b.bg}`}>
          <span className={`select-none text-4xl font-bold leading-none ${b.text}`}>
      ...
      {pulse && (
        <div
          className={`absolute z-10 flex h-2 w-2 ${showBand ? 'top-1.5 right-1.5' : 'top-2.5 right-2.5'}`}
        >
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {version && (
                <span className="shrink-0 rounded border border-border bg-muted px-1.5 py-px font-mono text-[10px] text-muted-foreground">
                  v{version}
                </span>
              )}
            </div>
      ...
      <div className="flex min-h-11 items-center justify-end gap-2 border-t border-border bg-muted/50 px-4 py-2.5">
        {actions}
      </div>
    </div>
  )
...
function PluginToggle({
  ...
  return (
    ...
    >
      <span
        className={`inline-block h-4 w-4 rounded-full bg-white transition-transform duration-200 ${enabled ? 'translate-x-4' : 'translate-x-0'}`}
      />
    </button>
  )
}
...
const PluginManager: React.FC<{ onClose: () => void }> = ({ onClose }) => {
  ...
  const load = useCallback(async () => {
    try {
      ...
      try {
        ...
        setBuiltinDrivers(d.filter(x => x.builtin))
      } catch {}
    } catch (err) {
      console.error(err)
  ...
  const filtered = useMemo(() => {
    ...
    if (activeFilter === 'all') {
      /* no filter — show all plugins */
    } else if (activeFilter === 'installed')
      list = list.filter(p => !!p.installed_version || isInstalled(p.id))
  ...
  const updateCount = useMemo(
    () => registryPlugins.filter(p => p.update_available).length,
    [registryPlugins]
  )
  ...
  const doInstall = async (pluginId: string, version: string) => {
    try {
      ...
      console.error(err)
      setRegistryError(t('plugin.installError') + ': ' + String(err))
    } finally {
  ...
  const doRemove = async (pluginId: string) => {
    try {
      ...
      await showMessage(t('plugin.confirmRemove'))
  ...
  const doToggle = async (pluginId: string, enabled: boolean) => {
    try {
      await (enabled ? disablePlugin(pluginId) : enablePlugin(pluginId))
      await load()
    } catch (err) {
      console.error(err)
    }
    ...
    <div className="flex flex-col h-full bg-background text-foreground">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={load}
            className="inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg border border-border bg-card text-xs text-muted-foreground hover:text-foreground hover:border-accent transition-colors"
          >
            <RefreshCw size={13} />
            {t('common.refresh')}
          </button>
        ...
        <div className="space-y-8 p-6">
          ...
          <div>
            ...
            <div className="flex items-center justify-between border-b border-border">
                ...
                {[
                  {
                    id: 'all' as const,
                    label: t('plugin.all'),
                    count: registryPlugins.filter(p => !p.installed_version && !isInstalled(p.id))
                      .length,
                  },
                  {
                    id: 'installed' as const,
                    label: t('plugin.installedTab'),
                    count:
                      builtinDrivers.length +
                      installedPlugins.filter(p => !builtinDrivers.some(d => d.id === p.id)).length,
                  },
                  ...
                  >
                    ...
                    {count > 0 && (
                      <span
                        ...
                        ${id === 'updates' && count > 0 ? 'bg-amber-500/20 text-amber-400' : 'bg-muted text-muted-foreground'}`}
                      >
                        {count}
                      </span>
                    )}
              ...
              <button
                onClick={load}
                className="mb-px flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors"
              >
                <RefreshCw size={12} />
                {t('common.refresh')}
              </button>
                ...
                <div className="grid gap-4 xl:grid-cols-2 lg:grid-cols-2 sm:grid-cols-1">
                    ...
                    .map(plugin => {
                      ...
                      return (
                        <PluginCard
                          key={plugin.id}
                          name={plugin.name}
                          description={plugin.description || (rp?.description ?? '')}
                          version={plugin.version}
                          author={rp?.author}
                          homepage={rp?.homepage}
                          accent={enabled ? 'blue' : null}
                          showBand
                          status={
                            <PluginToggle
                              enabled={enabled}
                              onToggle={() => doToggle(plugin.id, enabled)}
                            />
                          }
                          actions={
                            <button
                              onClick={() => doRemove(plugin.id)}
                  ...
                  {builtinDrivers.length === 0 && installedPlugins.length === 0 && (
                    <div className="col-span-2 flex flex-col items-center py-16 text-muted-foreground">
                      ...
                      <p className="text-sm">{t('plugin.noInstalled')}</p>
                    </div>
                  )}
                </div>
              ) : (
                <div className="grid gap-4 xl:grid-cols-2 lg:grid-cols-2 sm:grid-cols-1">
                        ...
                        actions={
                          installed ? (
                            <div className="flex items-center justify-end gap-2 w-full">
                              {hasUpdate && (
                                <button
                                  onClick={() => doInstall(plugin.id, plugin.latest_version)}
                  ...
                  {filtered.length === 0 && (
                    <div className="col-span-2 flex flex-col items-center py-16 text-muted-foreground">
                      ...
                      <p className="text-sm">
                        {activeFilter === 'updates' ? t('plugin.upToDate') : t('plugin.noResults')}
                      </p>
                    </div>
                  )}
                </div>
