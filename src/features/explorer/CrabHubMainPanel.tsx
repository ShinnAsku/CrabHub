...
import { useTabStore } from '@/stores/app-store'
import type {
  SchemaNode,
  Connection,
  ColumnInfo,
...
import {
  ...
  generateCopyTableName,
  buildDuplicateTableSQL,
} from '@/lib/export'
import EditorPanel from '@/features/editor/EditorPanel'
import MainPanelTabBar from '@/features/explorer/main-panel/MainPanelTabBar'
import ObjectListView from '@/features/explorer/main-panel/ObjectListView'
import TableDataView from '@/features/explorer/main-panel/TableDataView'
import TableContextMenu from '@/features/explorer/main-panel/TableContextMenu'
...
function CrabHubMainPanel({
  activeConnection,
  selectedSchemaName: propsSelectedSchemaName,
}: CrabHubMainPanelProps) {
  const {
    selectedSchemaName,
    selectedTable,
    selectedTableId,
  ...
  const [tableDataCache, setTableDataCache] = useState<
    Record<string, { data: QueryResult; ddl: string }>
  >({})
  ...
  const activeTableTab =
    activeTab?.type === 'table' && activeTab.tableId ? (activeTab as TableTab) : null
  ...
  useEffect(() => {
    ...
    const contextConnectionId = selectedContext?.connectionId
    const connId = contextConnectionId?.startsWith(`${activeConnection.id}:sub:`)
      ? contextConnectionId
      : activeConnection.id
    let cancelled = false
    setLoadedTables([])
  ...
  }, [activeConnection, currentSchemaName, selectedContext?.connectionId])
  ...
  const loadTableData = useCallback(
    async (
      ...
      log.debug(
        '[CrabHubMainPanel] loadTableData:',
        table.name,
        'schema:',
        resolvedSchema,
        'page:',
      ...
      try {
        ...
        if (result.rows.length === 0 && effectivePage > 1) {
          setPaginationState(prev => ({
            ...prev,
            [table.id]: { currentPage: 1, pageSize: effectivePageSize },
          }))
          setLoading(false)
        ...
        useExplorerStore.getState().setSelectedTableData(result)
        useExplorerStore.getState().setSelectedTableDDL(ddl)
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      for (const [rowIdx, changes] of editedRows.entries()) {
        ...
        const colsForWhere = (
          tableColumnInfoMap[activeTableTab.tableId] || selectedTableData.columns
        ).map((c: ColumnInfo) => ({
          name: c.name,
      ...
      for (const row of newRows) {
        const values: [string, any][] = Object.entries(row).filter(
  ...
  const handlePageChange = useCallback(
    (page: number) => {
      ...
      if (hasPendingChanges) {
        setConfirmDialog({
          message: t('pagination.unsavedWarning'),
          onConfirm: () => {
            setConfirmDialog(null)
            doChange()
          },
    ...
    async (format: 'csv' | 'json' | 'sql') => {
      ...
      try {
        ...
        if (isTauri) {
          ...
          if (!filePath) return
          const path =
            typeof filePath === 'string' ? filePath : ((filePath as any).path ?? String(filePath))
        ...
        if (format === 'sql') {
          ...
          setDataMessage({
            type: 'success',
            text: t('data.importSuccess').replace('{count}', String(imported)),
          })
        }
  ...
  const handleOpenTableTab = useCallback(
    (table: SchemaNode, connectionId?: string) => {
      const connId = connectionId || selectedContext?.connectionId || activeConnection?.id
      if (!connId) return

      const resolvedSchema =
        table.schemaName || selectedContext?.schemaName || currentSchemaName || 'public'
  ...
  const handleTableSelect = useCallback(
    async (table: SchemaNode) => {
      ...
      exportTableSql(connId, table.name, resolvedSchema)
        ...
        .catch(() => {
          setPreviewDDL('-- DDL not available')
        })
        .finally(() => {
          setDdlLoading(false)
        })
  ...
  useEffect(() => {
    if (selectedTable && (activeConnection || selectedContext?.connectionId)) {
      ...
      if (
        ...
        !tabs.find(
          t =>
            t.type === 'table' && t.tableId === (selectedTableId || `table-${selectedTable.name}`)
        )
      ) {
        prevSelectedTableRef.current = tableKey
  ...
  useEffect(() => {
    if (
      selectedContext?.type === 'table' &&
      selectedContext.tableName &&
      ...
      if (prevContextRef.current !== contextKey) {
        ...
        Promise.all([
          ...
          exportTableSql(connId, selectedContext.tableName, schema).catch(
            () => '-- DDL not available'
          ),
        ])
          .then(([columns, ddl]) => {
  ...
  useEffect(() => {
    if (activeTabId && activeTab?.type === 'table' && activeTab.tableId) {
      ...
      if (cached) {
        useExplorerStore.getState().setSelectedTableData(cached.data)
        useExplorerStore.getState().setSelectedTableDDL(cached.ddl)
      }
    }
  ...
  const handleDeleteTableAction = useCallback(
    (table: SchemaNode) => {
      ...
      const isNoSQL =
        ...
        activeConnection.type.startsWith('plugin:')
      setConfirmDialog({
        title: t('common.confirm'),
        message: isNoSQL
          ? `Delete key "${table.name}"?`
          : t('sidebar.confirmDeleteTable', { name: table.name }),
        variant: 'destructive',
        onConfirm: async () => {
          setConfirmDialog(null)
          try {
              ...
              const tableNodes: SchemaNode[] = result
                ...
                .map(ti => {
                  ...
                  return {
                    id,
                    name: ti.name,
                    type: (ti.tableType === 'VIEW' ? 'view' : 'table') as SchemaNode['type'],
  ...
  const handleTruncateTableAction = useCallback(
    (table: SchemaNode) => {
      if (!activeConnection) return
      setConfirmDialog({
        title: t('common.confirm'),
        ...
        onConfirm: async () => {
          ...
          try {
            ...
            const sql =
              dbType === 'sqlite' ? `DELETE FROM ${fullName}` : `TRUNCATE TABLE ${fullName}`
  ...
  const handleDuplicateTable = useCallback(
    async (table: SchemaNode, includeData: boolean) => {
      if (!activeConnection) return
        ...
        const tableNodes: SchemaNode[] = result
          ...
          .map(ti => {
            ...
            return {
              ...
              name: ti.name,
              type: (ti.tableType === 'VIEW' ? 'view' : 'table') as SchemaNode['type'],
              schemaName: ti.schema || currentSchemaName || 'public',
            }
          })
        setLoadedTables(tableNodes)
        setTableMetadataMap(metaMap)
