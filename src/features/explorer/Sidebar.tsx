...
import QueryHistory from '@/features/editor/QueryHistory'
import CreateDatabaseDialog from '@/features/connections/CreateDatabaseDialog'
import DatabaseIcon from '@/components/DatabaseIcon'
import { generateCopyTableName, buildDuplicateTableSQL } from '@/lib/export'
import { log } from '@/lib/log'
import { ConfirmDialog } from '@/components/ConfirmDialog'
import type { SidebarView, TreeNode, TreeNodeType } from '@/features/explorer/sidebar/types'
import { ContextMenu, TreeNodeContextMenu } from '@/features/explorer/sidebar/ContextMenus'
...
function Sidebar({ openConnectionDialog }: SidebarProps) {
  ...
  const [treeNodeContextMenu, setTreeNodeContextMenu] = useState<{
    x: number
    y: number
    node: TreeNode
  } | null>(null)
  ...
  const { schemaData, setSchemaData } = useExplorerStore()
  ...
  const handleRefreshNode = async (node: TreeNode) => {
    ...
    if (node.connectionId) {
      try {
        await invalidateMetadataCache(node.connectionId)
      } catch {
        /* best-effort */
      }
    }
  ...
  const handleDatabaseCreated = async (connectionId: string) => {
    ...
    try {
      await invalidateMetadataCache(connectionId)
    } catch {
      /* best-effort */
    }
  ...
  const handleNewQuery = (node: TreeNode) => {
    ...
    const queryCount =
      useTabStore.getState().tabs.filter((tab: any) => tab.type === 'query').length + 1
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          const sql =
            dbType === 'sqlite' ? `DELETE FROM ${tableName}` : `TRUNCATE TABLE ${tableName}`
  ...
  const handleOpenTable = (node: TreeNode) => {
    ...
    useConnectionStore.getState().setActiveConnection(connId)
    if (node.schemaName) {
      useExplorerStore.getState().setSelectedSchemaName(node.schemaName)
    }
    const tableInfo = {
      oid: null,
      name: node.name,
    ...
    useExplorerStore.getState().setSelectedTable(tableInfo)
    useExplorerStore.getState().setSelectedTableId(node.id)
    useExplorerStore.getState().setSelectedContext({
      type: 'table',
      connectionId: connId,
...
function ConnectionList({
  ...
  const connectAndExpand = async (connection: Connection) => {
    ...
    try {
      ...
      useConnectionStore
        .getState()
        .updateConnectionStatus(connection.id, { connected: true, lastConnected: new Date() })
  ...
  const handleConnectionClick = async (connectionId: string) => {
    ...
    setActiveConnection(connectionId)
    useExplorerStore.getState().setSelectedContext({ type: 'connection', connectionId })
  ...
  const handleDoubleClick = async (connection: Connection) => {
    ...
    if (connection.connected) {
      ...
      useExplorerStore
        .getState()
        .setSelectedContext({ type: 'connection', connectionId: connection.id })
  ...
  const handleContextMenu = (e: React.MouseEvent, connectionId: string) => {
    ...
    const connection = connections.find(c => c.id === connectionId)
    log.debug(
      `[Sidebar] handleContextMenu: 右键菜单打开 -> ${connection?.name || connectionId}, 位置=(${e.clientX}, ${e.clientY})`
    )
    setContextMenu({
      x: e.clientX,
      y: e.clientY,
...
function getCategoryName(cat: string, dbType: string): string {
  if (
    cat === 'tables' &&
    (dbType === 'redis' || dbType === 'mongodb' || dbType.startsWith('plugin:'))
  ) {
    return 'Keys'
  }
...
function DatabaseTree({
  ...
  const { schemaData } = useExplorerStore()
  ...
  const handleToggleNode = async (node: TreeNode, e: React.MouseEvent) => {
    ...
    if (node.type === 'schema') {
      ...
      useExplorerStore.getState().setSelectedSchemaName(node.name)
      useExplorerStore.getState().setSelectedContext({
        type: 'schema',
      ...
      ['tables', 'views', 'functions', 'procedures', 'events', 'triggers'].includes(node.type)
    ) {
      useConnectionStore.getState().setActiveConnection(connectionId)
      useExplorerStore.getState().setSelectedContext({
        type: 'folder',
  ...
  const handleTableClick = (table: TreeNode) => {
    ...
    useConnectionStore.getState().setActiveConnection(connectionId)
    if (table.schemaName) {
      useExplorerStore.getState().setSelectedSchemaName(table.schemaName)
    }
    useExplorerStore.getState().setSelectedContext({
      type: 'table',
      connectionId: table.connectionId || connectionId,
      schemaName: table.schemaName || undefined,
      tableName: table.name,
    })
    useExplorerStore.getState().setSelectedTableId(table.id)
  }
        ...
        if (row) {
          ...
          const def =
            ...
            Object.values(row).find(
              (v: unknown) => typeof v === 'string' && (v as string).length > 20
            ) ||
            ''
          content += String(def)
        }
  ...
  const loadDatabaseChildren = async (node: TreeNode) => {
    ...
    try {
      ...
      if (node.type === 'database') {
        ...
        if (needsSchemas) {
          ...
          try {
            ...
            children = schemaNames.map(name => ({
              ...
              name,
              connectionId: `${connectionId}:sub:${node.name}`,
        ...
        const targetConnectionId = node.connectionId || connectionId
        const tables = await getTables(targetConnectionId)
        ...
        children = filtered.map((table: any) => ({
          id: `${targetConnectionId}-${node.schemaName || 'default'}-table-${table.name}`,
          type: 'table',
          name: table.name,
        ...
        try {
          const targetConnectionId = node.connectionId || connectionId
          const views = await getViews(targetConnectionId, node.schemaName || undefined)
          log.debug('[DatabaseTree] Loaded', views.length, 'views')
          children = views.map((view: any) => ({
            id: `${targetConnectionId}-${node.schemaName || 'default'}-view-${view.name}`,
            type: 'view',
            name: view.name,
            connectionId: targetConnectionId,
            schemaName: node.schemaName,
        ...
        try {
          ...
          if (connection.type === 'gaussdb' || connection.type === 'opengauss') {
            ...
            result = await executeQuery(
              connectionId,
              `SELECT name FROM system.functions WHERE database = '${schema}' AND origin = 'SQL' ORDER BY name`
            )
          }
...
function ConnectionItem({
  ...
  const handleConnect = async (e: React.MouseEvent) => {
    e.stopPropagation()
    log.debug(
      '[ConnectionItem] Connect/Disconnect button clicked for:',
      connection.name,
      'Current status:',
    ...
    if (connection.connected) {
      ...
      useConnectionStore.getState().updateConnectionStatus(connection.id, { connected: false })
  ...
  return (
    ...
    >
      ...
      {/* Connection name */}
      <span
        className={`flex-1 truncate ${!connection.connected ? 'text-muted-foreground/50' : ''}`}
      >
        {connection.name}
      </span>
      ...
      <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
        {isConnecting ? (
          ...
          <>
            ...
            <button
              aria-label={t('sidebar.deleteConnection')}
...
function EmptyConnectionList({ openConnectionDialog }: { openConnectionDialog: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-xs px-4 text-center">
      ...
      <p className="text-[11px] text-muted-foreground/60 mb-3">{t('sidebar.noConnectionsHint')}</p>
      <button
        onClick={() => openConnectionDialog()}
