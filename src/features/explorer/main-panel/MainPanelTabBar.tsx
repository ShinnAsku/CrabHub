...
function tabIcon(type: Tab['type']) {
  switch (type) {
    case 'query':
      return <Code2 size={13} style={{ color: 'hsl(var(--tree-view))' }} />
    case 'table':
      return <Table size={13} style={{ color: 'hsl(var(--tree-table))' }} />
    case 'designer':
      return <PenTool size={13} style={{ color: 'hsl(var(--tree-function))' }} />
    case 'er':
      return <GitBranch size={13} style={{ color: 'hsl(var(--tree-schema))' }} />
    case 'notebook':
      return <NotebookText size={13} style={{ color: 'hsl(var(--tree-trigger))' }} />
    case 'query-builder':
      return <Table size={13} style={{ color: 'hsl(var(--tree-procedure))' }} />
    case 'analyzer':
      return <Brain size={13} style={{ color: 'hsl(var(--tree-procedure))' }} />
    default:
...
export default function MainPanelTabBar({
  ...
  const closeOthers = (keepId: string) => {
    tabs
      .filter(tb => tb.id !== keepId)
      .forEach(tb => onCloseTab(tb.id, new MouseEvent('click') as unknown as React.MouseEvent))
    setCtxMenu(null)
  }
    ...
    <div className="flex items-center border-b border-border bg-muted/30 shrink-0 overflow-x-auto overflow-y-hidden">
      ...
      {ctxMenu && (
        ...
        >
          ...
          <button
            className="w-full px-3 py-1.5 text-xs text-left hover:bg-muted"
            onClick={() => closeRight(ctxMenu.tabId)}
          >
            {t('tab.closeRight')}
          </button>
