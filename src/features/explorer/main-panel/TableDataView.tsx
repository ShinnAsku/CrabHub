...
import { Plus, Save, X, Trash2, Download, Upload, RefreshCw, Loader2, Check } from 'lucide-react'
import type { QueryResult, ColumnInfo, TableRow, TableTab } from '@/types/index'
import { t } from '@/lib/i18n'
import PaginationBar from '@/components/PaginationBar'
import TableDataError from '@/features/explorer/main-panel/TableDataError'
...
export default function TableDataView({
  ...
  useEffect(() => {
    if (newRows.length > prevNewRowsLen.current) {
      setTimeout(
        () => newRowsEndRef.current?.scrollIntoView({ behavior: 'smooth', block: 'end' }),
        50
      )
    }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="flex items-center gap-1 px-2 py-1 border-b border-border">
        ...
        {dataMessage && (
          <span
            className={`text-xs flex items-center gap-1 ${dataMessage.type === 'error' ? 'text-destructive' : 'text-success'}`}
          >
            ...
            >
              <tr>
                <th className="px-1 py-1 text-center border border-white/30 relative">
                  <input
                    ...
                    checked={
                      ...
                      selectedRowIndices.size === selectedTableData.rows.length + newRows.length
                    }
                    onChange={e => {
                      if (e.target.checked) {
                        const all = new Set<number>()
                        for (let i = 0; i < selectedTableData.rows.length + newRows.length; i++)
                          all.add(i)
                ...
                {selectedTableData.columns.map((col: ColumnInfo, idx: number) => (
                  <th
                    key={idx}
                    className="text-left px-2 py-1 font-medium text-white border border-white/30 relative group"
                    title={col.name}
                  >
              ...
              {newRows.map((row, nIdx) => {
                ...
                return (
                  ...
                  >
                    ...
                    {selectedTableData.columns.map((col: ColumnInfo, colIdx: number) => {
                      const isEditing =
                        editingCell?.rowIdx === globalIdx && editingCell?.colName === col.name
      ...
      {selectedTableData && (
        <PaginationBar
          ...
          totalPages={Math.max(
            1,
            Math.ceil(
              (totalRowCountCache[activeTableTab.tableId] ?? 0) /
                (paginationState[activeTableTab.tableId]?.pageSize || 1000)
            )
          )}
