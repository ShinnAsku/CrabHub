import { Edit, Plus, Trash2, Search, Eye, Table, Key } from 'lucide-react'
import { memo } from 'react'
import type { Connection, SchemaNode, ColumnInfo, TableInfo } from '@/types/index'
...
const isNoSQL = (type: string) =>
  type === 'redis' || type === 'mongodb' || type.startsWith('plugin:')
      ...
      <div className="flex-1 min-h-0 overflow-y-auto">
        <table className="w-full text-xs border-collapse border" style={{ tableLayout: 'fixed' }}>
          ...
          <thead className="sticky top-0" style={{ backgroundColor: 'hsl(var(--tab-active))' }}>
            {isNoSQL(activeConnection?.type || '') ? (
              <tr>
                <th className="text-left px-2 py-1 font-medium text-white truncate border border-white/30">
                  {t('common.name')}
                </th>
                <th className="text-left px-2 py-1 font-medium text-white truncate border border-white/30">
                  Type
              ...
              <tr>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('common.name')}
                >
                  {t('common.name')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.rows')}
                >
                  {t('tableHeader.rows')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.dataLength')}
                >
                  {t('tableHeader.dataLength')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.engine')}
                >
                  {t('tableHeader.engine')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.createdDate')}
                >
                  {t('tableHeader.createdDate')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.modifiedDate')}
                >
                  {t('tableHeader.modifiedDate')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.collation')}
                >
                  {t('tableHeader.collation')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.comment')}
                >
              ...
              <tr>
                ...
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.oid')}
                >
                  {t('tableHeader.oid')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.owner')}
                >
                  {t('tableHeader.owner')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.acl')}
                >
                  {t('tableHeader.acl')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.tableType')}
                >
                  {t('tableHeader.tableType')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.partitionOf')}
                >
                  {t('tableHeader.partitionOf')}
                </th>
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.rows')}
                ...
                <th
                  className="text-left px-2 py-1 font-medium text-white truncate border border-white/30"
                  title={t('tableHeader.primaryKey')}
                >
                  {t('tableHeader.primaryKey')}
                </th>
              </tr>
            )}
          </thead>
          <tbody>
            {tables.map(table => {
              ...
              if (isNoSQL(activeConnection?.type || '')) {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-1 truncate border">
                      <span className={typeColors[typeLabel] || 'text-muted-foreground'}>
                        {typeLabel}
                      </span>
                    </td>
                    <td className="px-2 py-1 text-muted-foreground truncate border">
                      {meta?.rowCount != null ? meta.rowCount : '—'}
                    </td>
              ...
              if (activeConnection?.type === 'mysql') {
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-2 py-1 text-muted-foreground truncate border">
                      {formatDataLength(meta?.dataLength)}
                    </td>
                    <td className="px-2 py-1 text-muted-foreground truncate border">
                      {meta?.engine ?? '—'}
                    </td>
              ...
              const tableTypeLabel =
                meta?.tableType === 'VIEW'
                  ? t('tableType.view')
                  : meta?.tableType === 'MATERIALIZED VIEW'
                    ? t('tableType.materializedView')
                    : meta?.tableType === 'PARTITIONED TABLE'
                      ? t('tableType.partitionedTable')
                      : meta?.tableType === 'FOREIGN TABLE'
                        ? t('tableType.foreignTable')
                        : t('tableType.regular')
              return (
                <tr
                  key={table.id}
      ...
      <div className="border-t border-border flex flex-col" style={{ height: '200px' }}>
        <div className="flex items-center justify-between px-2 py-1 border-b border-border bg-muted/20">
          <span className="text-xs font-medium text-foreground">
            {isNoSQL(activeConnection?.type || '') ? 'Value Structure' : t('columnHeader.columns')}
            {previewTableName && (
              <span className="text-muted-foreground"> - {previewTableName}</span>
            )}
          </span>
        </div>
          ...
          {columnsLoading ? (
            ...
            <table className="w-full text-xs border-collapse border">
              <thead className="sticky top-0" style={{ backgroundColor: 'hsl(var(--tab-active))' }}>
                <tr>
                  <th
                    className="text-left px-2 py-0.5 font-medium text-white border border-white/30 max-w-[150px] truncate"
                    title={t('columnHeader.name')}
                  >
                    {t('columnHeader.name')}
                  </th>
                  <th
                    className="text-left px-2 py-0.5 font-medium text-white border border-white/30 max-w-[120px] truncate"
                    title={t('columnHeader.type')}
                  >
                    {t('columnHeader.type')}
                  </th>
                  <th className="text-center px-1 py-0.5 font-medium text-white border border-white/30">
                    {t('columnHeader.pk')}
                  </th>
                  <th className="text-center px-1 py-0.5 font-medium text-white border border-white/30">
                    {t('columnHeader.nn')}
                  </th>
                  <th className="text-left px-2 py-0.5 font-medium text-white border border-white/30">
                    {t('columnHeader.defaultValue')}
                  </th>
                </tr>
              </thead>
              <tbody>
                {selectedColumns.map((col, idx) => (
                  <tr key={idx} className="hover:bg-muted/30">
                    ...
                    <td
                      className="px-2 py-0.5 text-muted-foreground border truncate max-w-[120px]"
                      title={col.type}
                    >
                      {col.type}
                    </td>
                    <td className="text-center px-1 py-0.5 border">
                      {col.primaryKey && (
                        <span className="text-amber-500 text-[11px] font-bold">PK</span>
                      )}
                    </td>
                    <td className="text-center px-1 py-0.5 border">
                      {col.notNull && (
                        <span className="text-blue-500 text-[11px] font-bold">NN</span>
                      )}
                    </td>
                    <td
                      className="px-2 py-0.5 text-muted-foreground truncate max-w-[80px] border"
                      title={col.defaultValue != null ? String(col.defaultValue) : ''}
                    >
