...
function DataMigration({ isOpen, onClose }: DataMigrationProps) {
  ...
  const loadSchemasForConnection = useCallback(
    async (connId: string, target: 'source' | 'target') => {
      if (!connId) return
  ...
  const handleGeneratePreview = useCallback(async () => {
    ...
    try {
      const { ddls, warnings } = await generateAllDDL(
        config,
        sourceConn,
        targetConn,
        selectedTables
      ...
      <div className="relative bg-background border border-border rounded-lg shadow-xl w-[800px] max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-foreground">{t('migration.title')}</span>
          </div>
          <button
            onClick={onClose}
          ...
          {[
            ...
            return (
              <div key={s.num} className="flex items-center gap-1 flex-1">
                ...
                {idx < 3 && <ChevronRight size={12} className="text-muted-foreground mx-0.5" />}
              </div>
            )
          })}
        </div>
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  ...
                  {dropExisting && <AlertTriangle size={12} className="text-yellow-500" />}
                </label>
              </div>
              ...
              <div className="p-3 bg-muted/50 rounded text-xs space-y-1">
                ...
                <div className="text-muted-foreground">
                  {sourceConn?.name} ({sourceConn?.type}) {t('migration.to')} {targetConn?.name} (
                  {targetConn?.type})
                </div>
                <div className="text-muted-foreground">
                ...
                <div className="text-muted-foreground">
                  {t('migration.mode')}:{' '}
                  {mode === 'structure'
                    ? t('migration.structureOnly')
                    : mode === 'data'
                      ? t('migration.dataOnly')
                      : t('migration.structureAndData')}
                </div>
              </div>
            </div>
          )}
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    ...
                    <div className="p-2 bg-muted/50 rounded text-center">
                      ...
                      <div className="text-sm font-semibold text-foreground">
                        {progress.rowsCopied.toLocaleString()}
                      </div>
                    </div>
                    <div className="p-2 bg-muted/50 rounded text-center">
                      <div className="text-[11px] text-muted-foreground">
                      ...
                      <div
                        className={`text-sm font-semibold ${progress.errors.length > 0 ? 'text-red-500' : 'text-green-500'}`}
                      >
