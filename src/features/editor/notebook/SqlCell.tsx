...
import { isDarkTheme, useUIStore } from '@/stores/modules/ui'
...
const SqlCell: React.FC<SqlCellProps> = ({
  ...
  return (
    <div
      className={`border rounded overflow-hidden ${isActive ? 'border-[hsl(var(--tab-active))]' : 'border-border'} ${isExpanded ? '' : 'h-8'}`}
    >
      {/* Cell Header */}
      <div className="flex items-center justify-between bg-muted/30 px-2 py-0.5">
        ...
        <div className="flex items-center gap-0.5">
          ...
          <button
            aria-label={t('notebook.addSqlCell')}
          ...
          <button
            aria-label={t('notebook.addMarkdownCell')}
                  ...
                  <div className="overflow-x-auto">
                      ...
                      <thead>
                        <tr>
                          {cell.result.columns.map((column: string, index: number) => (
                            <th
                              key={index}
                              className="px-2 py-0.5 text-left text-[11px] font-medium text-muted-foreground uppercase max-w-[300px]"
                            >
                    ...
                    {cell.result.rows.length > 100 && (
                      <div className="text-xs text-muted-foreground mt-2 text-center">
                        {t('notebook.showingPartial', {
                          count: 100,
                          total: cell.result.rows.length,
                        })}
                      </div>
                    )}
                  </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      {chartType === 'bar' && (
                        <BarChart data={chartData}>
                          ...
                          <XAxis
                            dataKey={xAxisColumn || (chartData[0] && Object.keys(chartData[0])[0])}
                          />
                          <YAxis />
                          <Tooltip
                            contentStyle={{
                      ...
                      {chartType === 'pie' && (
                        <PieChart>
                          <Pie
                            ...
                            dataKey="value"
                          >
                            {(pieData.length > 0 ? pieData : chartData.slice(0, 10)).map(
