...
import { ArrowDown, ArrowUp, Code, FileText, Plus, Settings, Trash2 } from 'lucide-react'
    ...
    return (
      <div className="prose prose-invert max-w-none">
        {content.split('\n').map((line, index) => {
          if (line.startsWith('# ')) {
            return (
              <h1 key={index} className="text-sm font-bold mb-1">
                {line.substring(2)}
              </h1>
            )
          } else if (line.startsWith('## ')) {
            return (
              <h2 key={index} className="text-xs font-bold mb-1">
                {line.substring(3)}
              </h2>
            )
          } else if (line.startsWith('### ')) {
            return (
              <h3 key={index} className="text-xs font-semibold mb-1">
                {line.substring(4)}
              </h3>
            )
          } else if (line.startsWith('- ')) {
            return (
              <li key={index} className="text-xs mb-0.5">
                {line.substring(2)}
              </li>
            )
          } else if (line === '') {
