...
import { useHistoryStore, useTabStore, useUIStore } from '@/stores/app-store'
import type { QueryHistoryEntry } from '@/types/index'
import { CheckCircle2, Clock, Search, Trash2, XCircle } from 'lucide-react'
import { useState } from 'react'
import { useShallow } from 'zustand/react/shallow'

function QueryHistory() {
  const { queryHistory, clearQueryHistory } = useHistoryStore(
    useShallow(state => ({
  ...
  if (queryHistory.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-full text-muted-foreground text-xs px-4 text-center">
        ...
        <p className="text-[10px] mt-1 text-muted-foreground/60">{t('history.noHistoryHint')}</p>
      </div>
    )
  }
