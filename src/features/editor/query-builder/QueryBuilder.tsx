import ColumnSelector from '@/features/editor/query-builder/ColumnSelector'
import FilterBuilder from '@/features/editor/query-builder/FilterBuilder'
import JoinBuilder from '@/features/editor/query-builder/JoinBuilder'
import QueryPreview from '@/features/editor/query-builder/QueryPreview'
import TableSelector from '@/features/editor/query-builder/TableSelector'
import { t } from '@/lib/i18n'
import { log } from '@/lib/log'
import { getColumns, getTables } from '@/lib/tauri-commands'
import { Filter, Loader2, Play, Table, X } from 'lucide-react'
...
const QueryBuilder: React.FC<QueryBuilderProps> = ({ connectionId, onClose, onQueryGenerated }) => {
  ...
  useEffect(() => {
    ...
    getTables(connectionId)
      .then(async tableList => {
        const result: Table[] = []
        for (const t of tableList.slice(0, 50)) {
          // limit to 50 tables
          try {
        ...
        if (result.length > 0) {
          ...
          setSelectedColumns(
            first.columns.slice(0, 5).map((c: Column) => `${first.name}.${c.name}`)
          )
        }
      })
      .catch(e => {
        log.error('QueryBuilder', 'Failed to load tables', e)
        setError(String(e))
      })
  ...
  const addJoin = useCallback(() => {
    const availableTables = tables.map(t => t.name).filter(t => !selectedTables.includes(t))
  ...
  const updateJoin = useCallback((id: string, updates: Partial<Join>) => {
    setJoins(prev => prev.map(join => (join.id === id ? { ...join, ...updates } : join)))
  }, [])
  ...
  const generateSQL = useCallback(() => {
    ...
    if (filters.length > 0) {
      sql += ' WHERE '
      sql += filters
        .map(filter => `${filter.column} ${filter.operator} '${filter.value.replace(/'/g, "''")}'`)
        .join(' AND ')
    }
