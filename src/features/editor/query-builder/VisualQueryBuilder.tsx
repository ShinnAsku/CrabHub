...
import { Database, Filter, GitMerge, Play, Plus, Settings, Table, Trash2, X } from 'lucide-react'
...
const TableNode = ({
  ...
  return (
    <div
      className={`border-2 rounded-lg shadow-lg bg-card min-w-[220px] ${selected ? 'border-[hsl(var(--tab-active))]' : 'border-border'}`}
    >
...
const JoinEdge = ({ data, selected }: EdgeProps<{ type: string; condition: string }>) => {
  return (
    <div
      className={`px-2 py-1 bg-card border border-border rounded text-xs ${selected ? 'border-[hsl(var(--tab-active))]' : ''}`}
    >
      {data?.type || 'JOIN'}
    </div>
  )
...
const VisualQueryBuilder: React.FC<QueryBuilderProps> = ({
  ...
  // Remove a table node
  const removeTableNode = useCallback(
    (nodeId: string) => {
  ...
  const updateJoin = useCallback(
    (edgeId: string, updates: Partial<{ type: string; condition: string }>) => {
      setEdges(prev =>
        prev.map(e => (e.id === edgeId ? { ...e, data: { ...e.data, ...updates } } : e))
      )
    },
    []
  )
  ...
  const generateSQL = useCallback(() => {
    ...
    if (filters.length > 0) {
      sql += ' WHERE '
      sql += filters
        .map(f => `${f.column} ${f.operator} '${f.value.replace(/'/g, "''")}'`)
        .join(' AND ')
    }
        ...
        <div className="flex-1 relative" ref={reactFlowWrapper}>
          ...
          >
            ...
            <Panel
              ...
              className="bg-card border border-border rounded-lg p-3 shadow-lg max-w-md"
            >
