...
const isPgLike = (type: DbType) => type === 'postgresql' || type === 'gaussdb'
...
function buildCreateDatabaseSQL(
  ...
  if (isPgLike(type)) {
    ...
    if (opts.allowConnections === false) clauses.push(`ALLOW_CONNECTIONS = false`)
...
function CreateDatabaseDialog({
    ...
    const loadOptions = async () => {
      ...
      try {
        if (isPgLike(connectionType)) {
          ...
          if (charsetResult.status === 'fulfilled' && charsetResult.value.rows) {
            const charsets = charsetResult.value.rows
              .map((row: Record<string, unknown>) => String(row['Charset'] || row['charset'] || ''))
              .filter(Boolean)
            setCharsetOptions(charsets)
          }

          if (collationResult.status === 'fulfilled' && collationResult.value.rows) {
            const collations = collationResult.value.rows.map((row: Record<string, unknown>) => ({
              collation: String(row['Collation'] || row['collation'] || ''),
  ...
  const filteredMysqlCollations = charset
    ? allMysqlCollations.filter(c => c.charset === charset).map(c => c.collation)
  ...
  const renderPgFields = () => (
    <>
      ...
      <div className="flex items-center gap-3">
        ...
        <select value={lcCtype} onChange={e => setLcCtype(e.target.value)} className={selectClass}>
          <option value="">{t('createDb.default')}</option>
          {collationOptions.map(c => (
            <option key={c} value={c}>
              {c}
      ...
      <div className="relative w-[480px] max-h-[85vh] bg-background border border-border rounded-lg shadow-xl flex flex-col">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-b border-border shrink-0">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-foreground">{t('createDb.title')}</span>
