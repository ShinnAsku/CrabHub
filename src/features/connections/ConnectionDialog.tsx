...
import { parsePoolInput } from '@/features/connections/profile'
...
const BUILTIN_DB_TYPES: { value: string; label: string; port: number; color: string }[] = [
  ...
  { value: 'oceanbase', label: 'OceanBase (MySQL)', port: 3306, color: '#0077C8' },
  { value: 'tidb', label: 'TiDB', port: 4000, color: '#E6005C' },
  { value: 'tdsql', label: 'TDSQL (MySQL)', port: 3306, color: '#0052D9' },
...
function ConnectionDialog({ isOpen, onClose, editConnection }: ConnectionDialogProps) {
  ...
  const isSingleSession = ['gaussdb', 'oracle', 'dameng', 'yashandb', 'gbase'].includes(type)

  /** Convert the four pool-option text inputs into the typed object. */
  const buildPoolOptions = (): ConnectionConfig['poolOptions'] => {
    if (isSingleSession) return undefined
    const out: NonNullable<ConnectionConfig['poolOptions']> = {
      maxConnections: parsePoolInput(poolMaxConnections),
      idleTimeoutSecs: parsePoolInput(poolIdleTimeoutSecs, true),
      maxLifetimeSecs: parsePoolInput(poolMaxLifetimeSecs, true),
      acquireTimeoutSecs: parsePoolInput(poolAcquireTimeoutSecs),
    }
  ...
  useEffect(() => {
    const loadPassword = async () => {
      if (isOpen && editConnection && editConnection.password == null) {
    ...
    if (isOpen) {
      if (editConnection) {
        ...
        setSslCaCert(editConnection.sslCerts?.caCert ?? '')
        setSslClientCert(editConnection.sslCerts?.clientCert ?? '')
        setSslClientKey(editConnection.sslCerts?.clientKey ?? '')
        ...
        setPoolAcquireTimeoutSecs(
          po?.acquireTimeoutSecs != null ? String(po.acquireTimeoutSecs) : ''
        )
    ...
    if (!editConnection) {
      ...
      if (dbType) {
        ...
        if (type === 'clickhouse') {
          ...
          setUsername('system')
          setDatabase('test')
        } else if (type === 'vastbase') {
          setUsername('')
          setDatabase('postgres')
        } else if (type === 'dameng') {
          setUsername('SYSDBA')
          setDatabase('')
        } else if (type === 'yashandb') {
          setUsername('sys')
          setDatabase('')
        } else if (type === 'gbase' || type === 'oracle') {
          setUsername('')
          setDatabase('')
  ...
  const handleTest = useCallback(async () => {
    if (type === 'oracle' && !database.trim()) {
      await showMessage(t('connection.oracleServiceRequired'))
      return
    }
    setTesting(true)
    setTestResult(null)
    try {
      const config: ConnectionConfig = {
        ...
        sslCerts: {
          caCert: sslCaCert || undefined,
          clientCert: sslClientCert || undefined,
          clientKey: sslClientKey || undefined,
        },
        keepaliveInterval,
        autoReconnect,
        queryTimeoutSecs,
      ...
      log.debug('Testing connection with', {
        name: config.name,
        type: config.type,
        host: config.host,
        port: config.port,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editConnection) {
        await updateConnection(editConnection.id, config)
      } else {
        await addConnection({ ...config, connected: false })
      }

      if (editConnection?.connected) {
        await disconnectDatabase(newId)
        useConnectionStore.getState().updateConnectionStatus(newId, { connected: false })
      }
      if (!editConnection || editConnection.connected) {
        try {
          await connectDatabase(config)
          useConnectionStore
            .getState()
            .updateConnectionStatus(newId, { connected: true, lastConnected: new Date() })
          setActiveConnection(newId)
        } catch (error) {
          useConnectionStore.getState().updateConnectionStatus(newId, { connected: false })
          await showMessage(
            `${t('connection.testFailedMsg')}: ${error instanceof Error ? error.message : String(error)}`
          )
        }
      }
      ...
      await showMessage(error instanceof Error ? error.message : String(error))
    } finally {
      setSaving(false)
                ...
                <div className="space-y-1">
                  ...
                  <div className="flex items-center gap-1.5">
                    <input
                      ...
                      placeholder={
                        sqliteMode === 'new'
                          ? t('connection.sqliteNewPlaceholder')
                          : '/path/to/database.db'
                      }
                    ...
                    />
                    <button
                      aria-label={t('connection.browse')}
              ...
              <>
                ...
                {[
                  ...
                  <div className="space-y-1">
                    <label className="text-xs text-muted-foreground">
                      {type === 'oracle'
                        ? t('connection.oracleServiceName')
                        : type === 'dameng'
                          ? 'Schema'
                    ...
                    <input
                      type="text"
                      aria-label={type === 'oracle' ? t('connection.oracleServiceName') : undefined}
                      ...
                      placeholder={
                        type === 'gaussdb'
                          ? 'gaussdb'
                          : ['postgresql', 'vastbase'].includes(type)
                            ? 'postgres'
                            : ''
                      }
                      className="w-full px-2.5 py-1.5 text-xs bg-muted border border-border rounded outline-none focus:border-[hsl(var(--tab-active))] transition-colors text-foreground placeholder:text-muted-foreground/60"
        ...
        return (
          <div className="space-y-3">
            <div className="space-y-2 p-2.5 bg-muted/30 rounded border border-border/50">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-[11px] text-muted-foreground/60">
                  ({t('connection.keepaliveHint')})
                </span>
              </div>
              <div className="flex items-center gap-2">
                <label className="text-[11px] text-muted-foreground whitespace-nowrap">
                  {t('connection.queryTimeout')}
                </label>
                <input
                  type="number"
            ...
            <fieldset
              ...
              className="space-y-2 p-2.5 bg-muted/30 rounded border border-border/50"
            >
              <label className="text-xs font-medium text-muted-foreground">
                {t('connection.poolTitle')}
              </label>
              <p className="text-[10px] text-muted-foreground/60">
                {t(isSingleSession ? 'connection.singleSession' : 'connection.poolHint')}
              </p>
              <div className="grid grid-cols-2 gap-2">
                <div className="flex items-center gap-2">
                  <label className="text-[11px] text-muted-foreground whitespace-nowrap w-28">
                    {t('connection.poolMaxConnections')}
                  </label>
                  <input
                    type="number"
                    value={isSingleSession ? '1' : poolMaxConnections}
      ...
      case 'database':
        return (
          <div className="space-y-3">
            <div className="space-y-1">
              <label className="text-xs text-muted-foreground">
                {t(type === 'oracle' ? 'connection.oracleServiceName' : 'connection.database')}
              </label>
              <input
                type="text"
                aria-label={type === 'oracle' ? t('connection.oracleServiceName') : undefined}
                value={database}
                onChange={e => setDatabase(e.target.value)}
                placeholder={type === 'oracle' ? '' : '(Optional)'}
                className="w-full px-2.5 py-1.5 text-xs bg-muted border border-border rounded outline-none focus:border-[hsl(var(--tab-active))] transition-colors text-foreground placeholder:text-muted-foreground/60"
              />
            </div>
          </div>
