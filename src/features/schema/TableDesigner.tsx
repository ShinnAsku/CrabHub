...
function TableDesigner({ connectionId, editTable }: TableDesignerProps) {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      log.debug(
        '[TableDesigner] Refreshing columns for:',
        editTable.name,
        'schema:',
        editTable.schema
      )
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="flex items-center justify-between px-3 py-1.5 border-b border-border shrink-0 gap-2">
        <div className="flex items-center gap-1">
          ...
          {/* Add Column */}
          <button
            aria-label={t('designer.addColumn')}
            onClick={addColumn}
      ...
      <div className="flex items-center px-3 border-b border-border shrink-0 gap-1">
        {designerTabs.map(tab => (
          <button
            ...
            style={
              activeDesignerTab === tab.key
                ? { backgroundColor: 'hsl(var(--tab-active))' }
                : undefined
            }
          >
            {tab.label}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-background border border-border rounded-lg shadow-xl p-5 w-[340px]">
            <h3 className="text-sm font-medium text-foreground mb-4">
              {t('designer.tableNameDialogTitle')}
            </h3>
            <div className="flex items-center gap-3 mb-5">
              <label className="text-xs text-foreground shrink-0">
                {t('designer.tableNameLabel')}
              </label>
              <input
                type="text"
                autoFocus
            ...
            >
              ...
              <td className="px-1 py-0.5 border-r">
                <select
                  ...
                  className="w-full px-1.5 py-0.5 text-xs bg-background border border-border hover:border-muted-foreground/30 focus:border-[hsl(var(--tab-active))] rounded outline-none text-foreground"
                >
                  {!allTypes.some(dt => dt.name.toLowerCase() === col.dataType.toLowerCase()) &&
                    col.dataType && <option value={col.dataType}>{col.dataType}</option>}
                  {allTypes.map(dt => (
...
function IndexesPanel({ indexes, isEditMode }: { indexes: any[]; isEditMode: boolean }) {
  ...
  return (
    <table className="w-full text-xs border-collapse">
      <thead className="sticky top-0 z-10">
        <tr style={{ backgroundColor: 'hsl(var(--tab-active))' }}>
          <th className="px-3 py-1.5 text-left font-medium border border-white/30 text-white">
            {t('designer.indexName')}
          </th>
          <th className="px-3 py-1.5 text-center font-medium border border-white/30 w-[60px] text-white">
            {t('designer.isUnique')}
          </th>
          <th className="px-3 py-1.5 text-center font-medium border border-white/30 w-[60px] text-white">
            {t('designer.isPrimary')}
          </th>
          <th className="px-3 py-1.5 text-left font-medium border border-white/30 text-white">
            {t('designer.indexColumns')}
          </th>
        </tr>
      </thead>
      <tbody>
          ...
          return (
            <tr key={i} className="border-b hover:bg-muted/30 transition-colors">
              ...
              <td className="px-3 py-1.5 text-center border-r">
                {isUnique ? <Check size={12} className="inline text-green-500" /> : ''}
              </td>
              <td className="px-3 py-1.5 text-center border-r">
                {isPrimary ? <KeyRound size={12} className="inline text-yellow-500" /> : ''}
              </td>
              <td className="px-3 py-1.5 text-muted-foreground">
                {typeof cols === 'string' ? cols : JSON.stringify(cols)}
              </td>
            </tr>
          )
        })}
  ...
  return (
    <table className="w-full text-xs border-collapse">
      <thead className="sticky top-0 z-10">
        <tr style={{ backgroundColor: 'hsl(var(--tab-active))' }}>
          <th className="px-3 py-1.5 text-left font-medium border border-white/30 text-white">
            {t('designer.constraintName')}
          </th>
          <th className="px-3 py-1.5 text-left font-medium border border-white/30 text-white">
            {t('designer.columnName')}
          </th>
          <th className="px-3 py-1.5 text-left font-medium border border-white/30 text-white">
            {t('designer.referencedTable')}
          </th>
          <th className="px-3 py-1.5 text-left font-medium border border-white/30 text-white">
            {t('designer.referencedColumn')}
          </th>
        </tr>
      </thead>
      <tbody>
...
function OptionsPanel({
  ...
  const inputClass =
    'px-2 py-1 text-xs bg-background border border-border rounded outline-none focus:border-[hsl(var(--tab-active))] text-foreground'
