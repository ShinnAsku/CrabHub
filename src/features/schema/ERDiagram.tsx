...
function layoutTables(tableData: { name: string; schema?: string; columns: TableColumn[] }[]): {
  tables: TableRect[]
  svgWidth: number
  svgHeight: number
} {
  ...
  const svgWidth =
    rects.length > 0 ? Math.max(800, ...rects.map(r => r.x + r.width + PADDING)) : 800
  const svgHeight =
    rects.length > 0 ? Math.max(400, ...rects.map(r => r.y + r.height + PADDING)) : 400
...
function buildConnections(tables: TableRect[], fks: FKConnection[]): ConnectionLine[] {
  ...
  for (const fk of fks) {
    ...
    const srcColIdx = src.columns.findIndex(
      c => c.name.toLowerCase() === fk.sourceColumn.toLowerCase()
    )
    const tgtColIdx = tgt.columns.findIndex(
      c => c.name.toLowerCase() === fk.targetColumn.toLowerCase()
    )
      ...
      {tables.map(table => (
        <g key={table.name}>
          ...
          <rect
            ...
            opacity="0.9"
          />
          <text x={table.x + 8} y={table.y + 16} fill="white" fontSize="11" fontWeight="bold">
            {table.name}
          </text>
...
function ERDiagram({ isOpen = true, onClose, connectionId, schemaName, embedded }: ERDiagramProps) {
          ...
          tableInfos.map(async ti => {
            ...
            for (const rawFk of rawFks) {
              ...
              if (normalized) {
                ...
                fkSourceColumns.add(
                  `${ti.name.toLowerCase()}.${normalized.sourceColumn.toLowerCase()}`
                )
              }
            }
  ...
  // Embedded mode
  if (embedded) {
    return <div className="w-full h-full overflow-auto bg-background">{content}</div>
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative w-[90vw] h-[85vh] bg-background border border-border rounded-lg shadow-xl flex flex-col">
        ...
        {/* SVG Canvas */}
        <div className="flex-1 overflow-auto bg-muted/10">{content}</div>
      </div>
    </div>
  )
}
